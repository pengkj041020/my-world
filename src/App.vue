<template>
  <div class="app-shell">
    <header class="navbar">
      ...
      <span v-else class="archive-label">世界档案</span>
    </header>

    <WorldDetail v-if="activeSection" :section="activeSection" @back="goHome" />
    ...
    <main v-else id="top">
      ...
      <section id="explore" class="section modules-section">
        ...
        <div class="card-grid">
          <a v-for="item in worldSections" :key="item.slug" class="module-card" :href="`#/${item.slug}`" @click="openSection(item.slug)">
...
import { updates, worldSections } from './data/worldSections'

const currentSlug = ref('')
const activeSection = computed(() => worldSections.find((item) => item.slug === currentSlug.value))
const syncRoute = () => { const slug = location.hash.replace(/^#\/?/, ''); currentSlug.value = worldSections.some((item) => item.slug === slug) ? slug : '' }
const openSection = (slug) => { currentSlug.value = slug; window.scrollTo({ top: 0, behavior: 'smooth' }) }
const goHome = () => { location.hash = '/'; currentSlug.value = ''; window.scrollTo({ top: 0, behavior: 'smooth' }) }
const showHome = () => { currentSlug.value = '' }
